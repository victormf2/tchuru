...
class InvalidRequestError(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["_tag", "message", "kind", "field"]

    @field_validator('tag')
    def tag_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['InvalidRequestError']):
            raise ValueError("must be one of enum values ('InvalidRequestError')")
        return value
