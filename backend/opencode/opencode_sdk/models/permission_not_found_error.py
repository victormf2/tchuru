...
class PermissionNotFoundError(BaseModel):
    ...
    message: StrictStr
    __properties: ClassVar[List[str]] = ["_tag", "requestID", "message"]

    @field_validator('tag')
    def tag_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PermissionNotFoundError']):
            raise ValueError("must be one of enum values ('PermissionNotFoundError')")
        return value
