...
class SymbolSource(BaseModel):
    ...
    range: Range
    name: StrictStr
    kind: Annotated[int, Field(strict=True, ge=0)]
    __properties: ClassVar[List[str]] = ["text", "type", "path", "range", "name", "kind"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['symbol']):
            raise ValueError("must be one of enum values ('symbol')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "text": FilePartSourceText.from_dict(obj["text"]) if obj.get("text") is not None else None,
            "type": obj.get("type"),
            "path": obj.get("path"),
