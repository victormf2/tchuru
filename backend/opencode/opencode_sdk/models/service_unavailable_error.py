...
class ServiceUnavailableError(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["_tag", "message", "service"]

    @field_validator('tag')
    def tag_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ServiceUnavailableError']):
            raise ValueError("must be one of enum values ('ServiceUnavailableError')")
        return value
