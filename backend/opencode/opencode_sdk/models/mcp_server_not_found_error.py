...
class McpServerNotFoundError(BaseModel):
    ...
    name: StrictStr
    message: StrictStr
    __properties: ClassVar[List[str]] = ["_tag", "name", "message"]

    @field_validator('tag')
    def tag_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['McpServerNotFoundError']):
            raise ValueError("must be one of enum values ('McpServerNotFoundError')")
        return value
