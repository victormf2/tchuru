...
class SessionNotFoundError(BaseModel):
    ...
    message: StrictStr
    __properties: ClassVar[List[str]] = ["_tag", "sessionID", "message"]

    @field_validator('tag')
    def tag_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SessionNotFoundError']):
            raise ValueError("must be one of enum values ('SessionNotFoundError')")
        return value
