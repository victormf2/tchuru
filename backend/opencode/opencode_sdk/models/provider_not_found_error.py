...
class ProviderNotFoundError(BaseModel):
    ...
    message: StrictStr
    __properties: ClassVar[List[str]] = ["_tag", "providerID", "message"]

    @field_validator('tag')
    def tag_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ProviderNotFoundError']):
            raise ValueError("must be one of enum values ('ProviderNotFoundError')")
        return value
