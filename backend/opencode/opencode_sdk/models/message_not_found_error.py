...
class MessageNotFoundError(BaseModel):
    ...
    message: StrictStr
    __properties: ClassVar[List[str]] = ["_tag", "sessionID", "messageID", "message"]

    @field_validator('tag')
    def tag_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['MessageNotFoundError']):
            raise ValueError("must be one of enum values ('MessageNotFoundError')")
        return value
