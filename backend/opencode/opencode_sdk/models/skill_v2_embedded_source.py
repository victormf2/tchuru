...
class SkillV2EmbeddedSource(BaseModel):
    """
    SkillV2EmbeddedSource
    """ # noqa: E501
    type: StrictStr
    skill: SkillV2Info
    __properties: ClassVar[List[str]] = ["type", "skill"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['embedded']):
            raise ValueError("must be one of enum values ('embedded')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "skill": SkillV2Info.from_dict(obj["skill"]) if obj.get("skill") is not None else None
        })
        return _obj
