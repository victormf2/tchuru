...
class ProviderAuthMethod(BaseModel):
    ...
    type: StrictStr
    label: StrictStr
    prompts: Optional[List[ProviderAuthMethodPromptsInner]] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["type", "label", "prompts"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['oauth', 'api']):
            raise ValueError("must be one of enum values ('oauth', 'api')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "label": obj.get("label"),
            "prompts": [ProviderAuthMethodPromptsInner.from_dict(_item) for _item in obj["prompts"]] if obj.get("prompts") is not None else None
        })
